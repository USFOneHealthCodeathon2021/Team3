...
data {
  ...
  vector<lower=0, upper=pi()>[N] theta_mu; // tip latitude estimate in radians
...
transformed parameters {
  ...
  vector[N+NI+NY] s = alpha + beta * y; // logistic likelihood of sampling a virus at a given location
...
model {
  ...
  0 ~ bernoulli_logit(segment(s, N+NI+1, NY)); // no observation at every point without a phylogenetic node (could be augmented with other random locations too, these are just convenient)
