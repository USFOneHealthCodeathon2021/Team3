...
functions {
  real spherical_cauchy_dots_lpdf(vector dots, vector scale) {
    vector[rows(scale)] ss = square(scale);
  ...
  vector dist_sphere_dots(vector dots) {
    int ND = rows(dots);
    vector[ND] d;
    for(n in 1:ND) {
  ...
  matrix gp_L(vector d, int N, real alpha, real rho, real sigma) {
    return(cholesky_decompose(
             fill_sym(square(alpha) * exp(-square(d) / (2 * square(rho))),
                      N,
                      square(alpha) + square(sigma))));
  }
}
data {
  ...
  int<lower=1, upper=NT+NI> self[NT+NI-1]; // index for each tip/node in a single vector
  int<lower=NT+1,upper=NT+NI> ancestor[NT+NI-1]; // index for each tip/node's ancestor in a single vector
...
transformed data {
  ...
  int iter = 1;
  for(n in 1:NT) kdonut[n] = fmax(kappa[n],100);
  for(j in 1:(N-1)) {
    for(i in (j+1):N) {
      dInd1[iter] = i;
      dInd2[iter] = j;
...
parameters {
  ...
  matrix[3,NN] locvec; // tip and node locations, unnormalized vector
...
transformed parameters {
  vector<lower=0>[NN] normalizing = sqrt(columns_dot_self(locvec))';
  matrix[3,N] loc = append_col(diag_post_multiply(locvec, inv(normalizing)), loc_y); // tip and node locations
...
model {
  ...
  y ~ multi_normal_cholesky(rep_vector(alpha_y, N), L_y); // gaussian process fit to observed covariates and predicts values at tip/node locations
