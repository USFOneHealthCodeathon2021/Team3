...
library(shiny);library(plotly);library(tidyverse);library(shinythemes)
...
library(mapproj)
library(cdlTools)
library(blscrapeR); library(DT) ; library(raster)
...
ui <-(
  navbarPage("Viral SpaceTime", id="main",
             ...
             tabPanel("About",includeMarkdown("Team3/APP_README.md"))
  ))
...
server <- function(input, output, session) {
  ...
  phydata <- read.csv("outputs/quick_alr/latlon_north_america_phy_divergence_rate - Copy.csv")
  dsn = "datasets/curated/census-app/"
  us.map <- readOGR(dsn = path.expand("datasets/curated/census-app/cb_2018_us_county_500k.shp"), layer="cb_2018_us_county_500k", stringsAsFactors = FALSE)
